    ...
    <Box sx={{ overflowX: "auto" }}>
      <svg width={W} height={H} role="img" aria-label="Daily cost chart" style={{ display: "block" }}>
        ...
        {Array.from({ length: yTicks + 1 }, (_, i) => {
          ...
          return (
            <g key={i}>
              <line x1={padX} y1={y} x2={padX + innerW} y2={y} stroke="rgba(0,0,0,0.08)" strokeWidth="1" />
              <text x={padX - 6} y={y + 3} fontSize="9" textAnchor="end" fill="rgba(0,0,0,0.5)">
          ...
          return (
            <g key={d.date}>
              ...
              <rect
                ...
                height={Math.max(0, h)}
                rx={2}
                fill={v > 0 ? "#FF7300" : "rgba(0,0,0,0.06)"}
                opacity={v > 0 ? 0.9 : 1}
              />
              {showLabel && (
                <text x={x + w / 2} y={H - 6} fontSize="9" textAnchor="middle" fill="rgba(0,0,0,0.5)">
                  {shortDate(d.date)}
                </text>
              )}
            </g>
      ...
      {rows.map((r) => {
        ...
        return (
          <Box key={r.model}>
            ...
            <Box sx={{ position: "relative", height: 8, borderRadius: 1, backgroundColor: "rgba(0,0,0,0.06)" }}>
              <Box
                sx={{
                  position: "absolute",
                  inset: 0,
              ...
              {inOutRatio ? (
                <Stack spacing={2}>
                  <Box>
                    ...
                    <Box sx={{ height: 10, borderRadius: 1, backgroundColor: "rgba(0,0,0,0.06)" }}>
              ...
              <Table size="small">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell align="right" sx={{ fontWeight: 600, color: "text.secondary", textTransform: "uppercase", fontSize: "0.7rem" }}>Tasks</TableCell>
                    ...
                    <TableCell align="right" sx={{ fontWeight: 600, color: "text.secondary", textTransform: "uppercase", fontSize: "0.7rem" }}>Calls</TableCell>
